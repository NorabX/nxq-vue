    ...
    Vue.mixin({
      methods: {
          ...
          if (y === undefined) {
            if (x === null || x === undefined) return undefined
              ...
              if(Array.isArray(x)) {
                ...
                else if(x.every((val) => val.q !== undefined)) return x
                else return undefined
              } else if(x.q === undefined) return this.createNXQObjects(x)
              else return x
            } else return undefined
          } else {
            if (typeof x === 'string') {
        ...
        __qHasClass: function(o) {
          o.qHasClass = function(v) {
            return o.classList.contains(v)
          }
        },

        // fd: qToggleClass(value)
        __qToggleClass: function(o) {
          o.qToggleClass = function(v) {
            o.classList.toggle(v)
          }
        },

        // fd: qReplaceClass(oldClassName, newClassName)
        __qReplaceClass: function(o) {
          o.qReplaceClass = function(ov, v) {
            o.classList.replace(ov, v)
          }
        },
        ...
        qQuery: function(elems, y, z) {
          ...
          for (let j = 0; j < elems.length; j++) {
            ...
            if (typeof y === 'string') {
              if (z === undefined) {
                if (elems.length == 1) res = this.__qQgetVal(elem, y)
                else res.push(this.__qQgetVal(elem, y))
              } else this.__qQsetVal(elem, y, z)
            } else {
              if (typeof y === 'object') {
                if (Array.isArray(y)) {
                  if (elems.length == 1) res = this.__qQgetVals(elem, y)
                  else res.push(this.__qQgetVals(elem, y))
        ...
        __qQgetVals: function(elem, y) {
          ...
          for (let i = 0; i < y.length; i++) {
            switch (y[i]) {
              ...
              case "children":
                values.push(elem.children)
                break
              case "parent":
                values.push(elem.parent)
                break
              case "first":
                values.push(elem.firstElementChild)
                break
              case "last":
                values.push(elem.lastElementChild)
                break
        ...
        __qQsetVals: function(elem, y, z) {
          ...
          for (let i = 0; i < props.length; i++) {
            switch (props[i]) {
              case "value":
                elem.value = y[props[i]]
                break
              case "text":
                elem.innerText = y[props[i]]
                break
              case "html":
                elem.innerHTML = y[props[i]]
                break
            }
          }
        },
